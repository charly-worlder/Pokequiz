...
- [x] T26  Sperrmeldung nach Ursache unterscheiden (BUG-67): `registerAttempt` gibt zurück, **welcher** Zähler abgewiesen hat; die Actions wählen daraufhin zwischen der Verbindungs- und der Adress-Meldung. Bei doppelter Sperre gewinnt die Adresse (längeres Fenster). **Nicht `[P]`:** dieselben Dateien wie T22/T24  · files: src/lib/auth/throttle.ts, src/lib/auth/actions.ts, src/lib/auth/error-mapping.ts, src/lib/auth/throttle.test.ts, src/lib/auth/actions.test.ts  · → AC-16, AC-19, EC-4
...
- **Grobkörnig, nicht mikro.** Aufgaben sind sinnvolle Prüfpunkte (25 in diesem Feature), keine Einzeiler.
