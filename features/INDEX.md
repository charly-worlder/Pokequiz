...
| PROJ-1 | **Die Drosselung braucht einen Host, der `x-forwarded-for` selbst setzt UND clientseitige Werte verwirft** — beide Richtungen, und beide sind Pflicht: **(a) BUG-53** — setzt der Host gar keinen Header, fallen **alle** Spieler auf den gemeinsamen Zählerschlüssel `unbekannt` mit 5 Versuchen pro Minute; der Schutz wird zur Aussperr-Waffe gegen die eigenen Nutzer. **(b) BUG-61 (High)** — *hängt* der Host nur an, statt zu überschreiben (der gewöhnliche nginx-Aufbau), bleibt der erste Eintrag angreiferkontrolliert und die IP-Hälfte ist abschaltbar: **gemessen 40 Rateversuche gegen 40 Konten in 5 s, null abgewiesen**, dazu 25 Konten in 9 s. Bis diese Richtung entschieden ist, ist BUG-61 ein **offener High**, kein reiner Deploy-Punkt | (a) beim Deploy · (b) **Entscheidung offen** |
